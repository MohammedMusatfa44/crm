...
try {
    ...
    // Test 5: Test trigger method
    echo "5. Testing trigger method...\n";
    $notification = \App\Models\Notification::where('user_id', $user->id)
        ->where('is_triggered', false)
        ->first();

    if ($notification) {
        echo "   Found notification to trigger: ID {$notification->id}\n";
        $request = new \Illuminate\Http\Request();
        $result = $controller->trigger($notification->id);
        $responseData = json_decode($result->getContent(), true);

        if ($responseData['success']) {
            echo "✅ Trigger method executed successfully\n";
        } else {
            echo "❌ Trigger method failed: {$responseData['message']}\n";
        }
    } else {
        echo "   No notifications found to trigger\n";
    }
    echo "\n";
